...
| 🎯 核心能力 | 说明 |
|---|---|
| 🔍 **全量扫描** | 桌面端索引库 ∪ CLI 会话库，显示标题、工作区、消息数、状态、更新时间 |
| 🗑️ **一键删除** | 每条对话最右侧删除按钮，二次确认后执行 |
| 🧨 **彻底清除** | 数据库行级删除 → WAL checkpoint + VACUUM 消除磁盘残留 → 级联删除关联文件 → 日志明文清洗 |
| 🛡️ **安全兜底** | 删除前自动备份数据库至 `~/.zcode-cleaner-backup/`，可随时恢复 |
| 🍎 **Apple 风格** | macOS 无边框窗口、红绿灯按钮、圆角卡片、渐入动画、精致质感 |
